...
GrafoLista *criarGrafoLista(int numeroVertices){
...
void criarImagemGrafoLista(GrafoLista *grafo, bool variasVersoes){
    ...
    if(variasVersoes){
        sprintf(nome, "./outputs/grafoLista-%d", qtdPrintsLista);
    }else{
...
void inserirArestaDirecionadaGrafoLista(GrafoLista *grafo, int verticeA, int verticeB, int peso){
    if(verticeA >= grafo->numeroVertices || verticeA < 0 || verticeB >= grafo->numeroVertices || verticeB < 0){
        printf("!! O(s) vértice(s) não existe(m)!!\n");
        return;
    }
...
void inserirArestaBidirecionadaGrafoLista(GrafoLista *grafo, int verticeA, int verticeB, int peso){
    if(verticeA >= grafo->numeroVertices || verticeA < 0 || verticeB >= grafo->numeroVertices || verticeB < 0){
        printf("!! O(s) vértice(s) não existe(m)!!\n");
        return;
    }
    ...
    if(inserirLista(&(grafo->arestas[verticeA]), verticeB, peso)){
        ...
        if(inserirLista(&(grafo->arestas[verticeB]), verticeA, peso)){
            grafo->quantidadeArestas++;
            printf("-> Aresta [%d] v%d <-> v%d adicionada\n", peso, verticeA, verticeB);
        }
    }
...
void removerArestaDirecionadaGrafoLista(GrafoLista *grafo, int verticeA, int verticeB){
    if(verticeA >= grafo->numeroVertices || verticeA < 0 || verticeB >= grafo->numeroVertices || verticeB < 0){
        printf("!! O(s) vértice(s) não existe(m)!\n");
        return;
    }
    ...
    if(removerLista(&(grafo->arestas[verticeA]), verticeB)){
        grafo->quantidadeArestas--;
        printf("-> Aresta v%d -> v%d removida\n", verticeA, verticeB);
    }
}

void removerArestaBidirecionadaGrafoLista(GrafoLista *grafo, int verticeA, int verticeB){
    if(verticeA >= grafo->numeroVertices || verticeA < 0 || verticeB >= grafo->numeroVertices || verticeB < 0){
        printf("!! O(s) vértice(s) não existe(m)!\n");
        return;
    }

    if(grafo->arestas[verticeA] == NULL && grafo->arestas[verticeB] == NULL){
        printf("-> O vértice [%d] não possui arestas com outros vértices\n", verticeA);
        return;
    }
    ...
    if(removerLista(&(grafo->arestas[verticeA]), verticeB)){
        ...
        if(removerLista(&(grafo->arestas[verticeB]), verticeA)){
            grafo->quantidadeArestas--;
            printf("-> Aresta v%d <-> v%d removida\n", verticeA, verticeB);
        }
    }
...
bool verificarVerticePossuiVizinho(GrafoLista *grafo, int vertice){
    ...
    for(int i = 0; i < grafo->numeroVertices; i++){     
        if(i != vertice){
            if(verificarArestaExisteGrafoLista(grafo, i, vertice)){
