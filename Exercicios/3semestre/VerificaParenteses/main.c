...
/*
A função recebe uma expressão aritmética e verifica se os colchetes e 
parênteses foram abertos e fechados corretamente.
Retorna 1 caso tenha erro, senão, retorna 0.
*/
int verificaExpressao(char *expressao){
    int ultimo; //caso exista erro, representará a ultima chave aberta
    ...
    for(int i = 0; i < strlen(expressao); i++){
        ...
        if(*letra == '('){
            ...
            if(pop(pilha) == 0){ //chaves fechando a mais
                ultimo = i;
                i = strlen(expressao);
                erro = 1;
            }
    ...
    }else{ //faltou chaves para fechar
