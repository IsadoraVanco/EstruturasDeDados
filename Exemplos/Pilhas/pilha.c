...
// Neste exemplo, a pilha é implementada como uma estrutura que contém um array de ponteiros 
// para os elementos, bem como informações sobre o tamanho atual da pilha e sua capacidade total. 
// As funções `stack_push`, `stack_pop`, `stack_top` e `stack_is_empty` realizam as operações usuais 
// de empilhar, desempilhar, obter o elemento no topo da pilha e verificar se a pilha está vazia, 
// respectivamente. A função `stack_init` é usada para inicializar a pilha com uma capacidade inicial 
// especificada, e a função `stack_free` é usada para desalocar a memória utilizada pela pilha.

// Este código é genérico, pois os elementos da pilha são ponteiros `void*`, o que significa que podem 
// ser usados para armazenar qualquer tipo de dado. No exemplo de uso da pilha no `main()`, são empilhados 
// um inteiro, um float e um char, e depois desempilhados e impressos na ordem inversa.